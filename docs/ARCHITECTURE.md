...
|------|----|----|-----|
| 2026-06-09 | — | First product module: `snowflake_pii_scanner/` Python package (config / catalog / rules / scanner / report / cli) + `pyproject.toml` + `tests/pii_scanner/` (pytest, 65 tests, no network) + `docs/PII_SCANNER.md`. Two-tier PII detection: metadata scan of INFORMATION_SCHEMA column names; opt-in `--deep` in-database value profiling via `REGEXP_LIKE` over `SAMPLE (N ROWS)` returning only aggregate counts. Credentials strictly from `SNOWFLAKE_*` env vars (password / key-pair / SSO). | Operator goal: build a Snowflake PII-detection CLI; raw PII must never leave Snowflake or reach logs/reports — guaranteed structurally by pushing regex matching into Snowflake. |
